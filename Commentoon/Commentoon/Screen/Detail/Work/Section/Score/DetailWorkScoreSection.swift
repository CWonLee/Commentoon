    ...
    var body: some View {
        VStack {
            ...
            HStack(spacing: 0) {
                ...
                VStack(spacing: 0) {
                    Spacer()
                    Text(String(format: "%.1f", viewModel.productModel?.averageScore ?? 0.0))
                        .font(.system(size: 24))
                        .foregroundColor(.black)
                    Spacer().frame(height: 10)
                    StarRatingView(rating: viewModel.productModel?.averageScore ?? 0.0)
                    Spacer().frame(height: 8)
                    Text("\(viewModel.productModel?.reviewCount ?? 0)개의 리뷰")
                        .font(.system(size: 12))
                ...
                VStack(spacing: 3) {
                    ForEach(stride(from: 5, to: 0, by: -1).reversed(), id: \.self) { index in
                        GraphView(progress: viewModel.productModel?.scoreGraph?[5-index] ?? 0, index: index)
                    }
                }
                Spacer()
            }
